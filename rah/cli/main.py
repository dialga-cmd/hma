"""
Command-line interface for RAH.
"""
...
from rah import RAH
from rah.core.exceptions import RAHError
...
def main():
    parser = argparse.ArgumentParser(
        description="RAH - Retrieval-Augmented Hierarchy. A smarter alternative to RAG."
    )
    ...
    try:
        if args.command == "index":
            ...
            print(f"Map saved to {os.path.join(args.path, '.rah.json')}")
            ...
            if not kmap:
                print(f"No knowledge map found for {args.project}. Run 'rah index' first.")
                sys.exit(1)
    ...
    except RAHError as e:
        print(f"❌ RAH Error: {e}", file=sys.stderr)
        sys.exit(1)
