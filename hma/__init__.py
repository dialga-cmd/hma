"""
HMA — Retrieval-Augmented Hierarchy
...
__version__ = "0.1.0"
__author__ = "HMA Contributors"

from hma.core.models import FileNode, Relationship, KnowledgeMap, Answer
from hma.core.config import HMAConfig
from hma.core.exceptions import (
    HMAError,
    IndexingError,
    QueryError,
    NoLLMError,
...
from hma.indexing.map_builder import MapBuilder
from hma.querying.answerer import QueryEngine
from hma.storage.json_store import JSONStore
...
class HMA:
    """
    Main entry point for the HMA library.

    HMA (Retrieval-Augmented Hierarchy) analyzes your project's file
    ...
    Args:
        llm: A callable that takes a prompt string and returns a response string.
             This is YOUR LLM — HMA doesn't bundle any model.
        config: Optional HMAConfig for customizing behavior.
    """

    def __init__(
        self,
        llm: Callable[[str], str],
        config: Optional[HMAConfig] = None,
    ):
        if not callable(llm):
            raise NoLLMError(
                "The 'llm' argument must be a callable that takes a prompt (str) "
                "and returns a response (str). HMA does not bundle any LLM — "
                "you must provide your own."
            )
        self.llm = llm
        self.config = config or HMAConfig()
    ...
    def index(
        ...
        and saves the map as .hma.json in the project root.
    ...
    def __repr__(self) -> str:
        ...
        return f"HMA(status={status}, version={__version__})"
